...
class Tester:
    ...
    def __call__(self, test_data, verbose=True, save_data=False, path='./data'):
        ...
        self.model.eval()
        with torch.no_grad():
            loss, nmse, SNR = self._iteration(test_data, save_data, path)
        if verbose:
            logger.info(f'\n=> Test result: \nloss: {loss:.3e}'
                  f'  NMSE: {nmse:.3e}'
    ...
    def _iteration(self, data_loader, save_data=False, path='./data'):
        ...
        for batch_idx, (sparse_gt, ) in enumerate(data_loader):
            ...
            sparse_pred, _, SNR = self.model(sparse_gt, save_data, path)
